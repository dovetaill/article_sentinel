...
func (r *TaskOutboxRelay) DispatchMessage(ctx context.Context, outboxID uint64) error {
	...
	if err != nil {
		_, updateErr := r.moveToDeadLetter(ctx, claimedMessage, now, TaskOutboxErrorPayloadDecode, err)
		if updateErr != nil {
			return updateErr
		}
		return err
	}
	if strings.TrimSpace(claimedMessage.MessageType) != TaskOutboxMessageTypeRunTask {
		_, updateErr := r.moveToDeadLetter(ctx, claimedMessage, now, TaskOutboxErrorUnsupportedMessage, errors.New("unsupported outbox message type"))
		if updateErr != nil {
			return updateErr
		}
		return errors.New("unsupported outbox message type")
	}
...
func (r *TaskOutboxRelay) DispatchPending(ctx context.Context, limit int) (TaskOutboxDispatchReport, error) {
	...
	for _, item := range items {
		...
		var updated InspectionTaskOutboxMessage
		...
		case TaskOutboxStatusDispatched:
			report.Dispatched++
		case TaskOutboxStatusPending:
			if updated.AttemptCount > beforeAttempts || statusBefore == TaskOutboxStatusClaimed {
				report.Retried++
			}
		case TaskOutboxStatusDeadLetter:
			report.DeadLettered++
		}
	}
	return report, nil
...
func (r *TaskOutboxRelay) claimMessage(ctx context.Context, message InspectionTaskOutboxMessage, now time.Time) (bool, InspectionTaskOutboxMessage, error) {
	if r == nil || r.db == nil {
		return false, InspectionTaskOutboxMessage{}, ErrInvalidTaskInput
	}
	if !isDispatchableMessage(message, now) {
	...
	query := r.db.WithContext(ctx).Model(&InspectionTaskOutboxMessage{}).Where("id = ?", message.ID)
	switch strings.TrimSpace(message.Status) {
	case TaskOutboxStatusPending:
		query = query.Where("status = ?", TaskOutboxStatusPending).
			Where("next_attempt_at IS NULL OR next_attempt_at <= ?", now)
	case TaskOutboxStatusClaimed:
		query = query.Where("status = ?", TaskOutboxStatusClaimed).
	...
	result := query.Updates(map[string]any{
		"status":      TaskOutboxStatusClaimed,
		"claimed_by":  r.claimOwner,
		"claimed_at":  now,
		"claim_until": leaseUntil,
	})
	if result.Error != nil {
		return false, InspectionTaskOutboxMessage{}, result.Error
	}
	if result.RowsAffected == 0 {
		return false, message, nil
	}

	message.Status = TaskOutboxStatusClaimed
...
func (r *TaskOutboxRelay) markDispatched(ctx context.Context, message InspectionTaskOutboxMessage, now time.Time) error {
...
func (r *TaskOutboxRelay) scheduleRetry(ctx context.Context, message InspectionTaskOutboxMessage, now time.Time, errorCode string, dispatchErr error) (int64, error) {
	attemptCount := message.AttemptCount + 1
	...
	return attemptCount, r.db.WithContext(ctx).Model(&InspectionTaskOutboxMessage{}).
		Where("id = ?", message.ID).
		Updates(map[string]any{
...
func (r *TaskOutboxRelay) moveToDeadLetter(ctx context.Context, message InspectionTaskOutboxMessage, now time.Time, errorCode string, dispatchErr error) (int64, error) {
	attemptCount := message.AttemptCount + 1
	...
	return attemptCount, r.db.WithContext(ctx).Model(&InspectionTaskOutboxMessage{}).
		Where("id = ?", message.ID).
		Updates(map[string]any{
			"status":           TaskOutboxStatusDeadLetter,
			"attempt_count":    attemptCount,
			"claimed_by":       "",
			"claimed_at":       nil,
			"claim_until":      nil,
...
func (r *TaskOutboxRelay) shouldDeadLetter(message InspectionTaskOutboxMessage, errorCode string) bool {
	if errorCode == TaskOutboxErrorPayloadDecode || errorCode == TaskOutboxErrorUnsupportedMessage {
		return true
	}
...
func classifyDispatchError(err error) string {
	...
	case errors.Is(err, ErrTaskOutboxDispatcherUnavailable):
		return TaskOutboxErrorDispatcherUnavailable
	default:
		return TaskOutboxErrorDispatch
	}
}
