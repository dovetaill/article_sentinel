...
func (s *CategoryService) PatchEnabled(ctx context.Context, input PatchCategoryStatusInput) (*CategoryDTO, error) {
	...
	operatorID, operatorName := auditOperatorFromContext(ctx)
	if err := s.repo.PatchEnabled(ctx, input.OrgID, input.CategoryID, input.Enabled, operatorID, operatorName); err != nil {
		return nil, err
	}
	return s.Get(ctx, input.OrgID, input.CategoryID)
}

func buildCategoryDTO(item InspectionCategory) CategoryDTO {
	return CategoryDTO{
		ID:          item.ID,
		OrgID:       item.OrgID,
		Name:        item.Name,
