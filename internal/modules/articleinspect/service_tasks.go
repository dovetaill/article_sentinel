...
func (s *TaskService) CreateWithOutbox(ctx context.Context, input CreateInspectionTaskInput) (*InspectionTask, *InspectionTaskOutboxMessage, error) {
	prepared, err := s.prepareTaskCreate(ctx, input)
	if err != nil {
		return nil, nil, err
	}

	outbox := &InspectionTaskOutboxMessage{}
	if err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (s *TaskService) deleteTaskGraph(ctx context.Context, orgID, taskID uint64) error {
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("orgid = ? AND task_id = ?", orgID, taskID).Delete(&InspectionTaskOutboxMessage{}).Error; err != nil {
			return err
		}
