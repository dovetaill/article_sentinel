...
func (s *KeywordService) Create(ctx context.Context, input CreateKeywordInput) (*KeywordDTO, error) {
	...
	keyword := &domainpkg.InspectionKeyword{
...
func (s *KeywordService) PatchEnabled(ctx context.Context, input PatchKeywordStatusInput) (*KeywordDTO, error) {
	...
	operator := sharedpkg.ResolveOperator(ctx)
	if err := s.repo.PatchEnabled(ctx, input.OrgID, input.KeywordID, input.Enabled, operator.ID, operator.Name); err != nil {
		return nil, err
	}
...
func buildKeywordScopes(orgID uint64, scopes []string) []domainpkg.InspectionKeywordScope {
	items := make([]domainpkg.InspectionKeywordScope, 0, len(scopes))
	for _, scope := range scopes {
		items = append(items, domainpkg.InspectionKeywordScope{OrgID: orgID, Scope: scope})
	}
	return items
}

func buildKeywordDTO(keyword *KeywordRecord, scopes []domainpkg.InspectionKeywordScope) *KeywordDTO {
	if keyword == nil {
		return nil
	}
