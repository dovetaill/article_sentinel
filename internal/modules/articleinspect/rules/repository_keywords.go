...
func (r *KeywordRepository) Create(ctx context.Context, keyword *domainpkg.InspectionKeyword, scopes []domainpkg.InspectionKeywordScope) error {
...
func (r *KeywordRepository) Update(ctx context.Context, keyword *domainpkg.InspectionKeyword, scopes []domainpkg.InspectionKeywordScope) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&domainpkg.InspectionKeyword{}).
		...
		if err := tx.Where("orgid = ? AND keyword_id = ?", keyword.OrgID, keyword.ID).Delete(&domainpkg.InspectionKeywordScope{}).Error; err != nil {
			return err
		}
		if len(scopes) == 0 {
			return nil
...
func (r *KeywordRepository) Delete(ctx context.Context, orgID, id uint64) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("orgid = ? AND keyword_id = ?", orgID, id).Delete(&domainpkg.InspectionKeywordScope{}).Error; err != nil {
			return err
		}
		result := tx.Where("orgid = ? AND id = ?", orgID, id).Delete(&domainpkg.InspectionKeyword{})
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
...
func (r *KeywordRepository) Get(ctx context.Context, orgID, id uint64) (*KeywordRecord, []domainpkg.InspectionKeywordScope, error) {
	if r == nil || r.db == nil || orgID == 0 || id == 0 {
...
func (r *KeywordRepository) List(ctx context.Context, filter KeywordListFilter) ([]KeywordRecord, map[uint64][]domainpkg.InspectionKeywordScope, int64, error) {
	if r == nil || r.db == nil || filter.OrgID == 0 {
		return nil, nil, 0, ErrInvalidKeywordInput
	}

	page, pageSize := sharedpkg.NormalizePage(filter.Page, filter.PageSize)
	countQuery := r.db.WithContext(ctx).Model(&domainpkg.InspectionKeyword{}).Where("orgid = ?", filter.OrgID)
	...
	if len(items) == 0 {
		return items, map[uint64][]domainpkg.InspectionKeywordScope{}, total, nil
	}
...
func (r *KeywordRepository) PatchEnabled(ctx context.Context, orgID, id uint64, enabled bool, updaterID uint64, updaterName string) error {
	...
	result := r.db.WithContext(ctx).Model(&domainpkg.InspectionKeyword{}).
...
func (r *KeywordRepository) ListByIDs(ctx context.Context, orgID uint64, ids []uint64) ([]KeywordRecord, map[uint64][]domainpkg.InspectionKeywordScope, error) {
...
func (r *KeywordRepository) listScopes(ctx context.Context, orgID uint64, keywordIDs []uint64) (map[uint64][]domainpkg.InspectionKeywordScope, error) {
	result := make(map[uint64][]domainpkg.InspectionKeywordScope, len(keywordIDs))
	if len(keywordIDs) == 0 {
		return result, nil
	}

	var scopes []domainpkg.InspectionKeywordScope
...
func ensureKeywordCategoryExists(tx *gorm.DB, orgID, categoryID uint64) error {
	...
	var count int64
	if err := tx.Model(&domainpkg.InspectionCategory{}).
