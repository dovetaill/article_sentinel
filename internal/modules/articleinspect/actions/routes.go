...
func registerBatchActionRoute(api huma.API, service *ActionService, path, operationID, summary string, handler func(context.Context, BatchActionInput) (*BatchActionSummary, error)) {
	huma.Register(api, huma.Operation{
		...
		Path:        path,
		Summary:     summary,
	}, func(ctx context.Context, input *batchActionRequest) (*sharedpkg.OKEnvelopeOutput, error) {
		orgID, err := sharedpkg.CurrentOrgID(ctx)
		if err != nil {
			return failureOKFromError(err)
		}
		if len(input.Body.ResultIDs) == 0 {
			return sharedpkg.FailureOKEnvelope(http.StatusBadRequest, "result_ids are required"), nil
		}
		...
		return sharedpkg.SuccessOKEnvelope(http.StatusOK, "batch action applied", result), nil
	})
}
